...
**Mobility profile enhancement: done** (`eda/har70_mobility_profile.py`). Reused the k-means clusters to compute, per subject, what % of their session was spent in each movement-intensity zone (sedentary/moderate/vigorous). Real, meaningful variation across all 18 real subjects — e.g. subject 505 is 89% sedentary / 0% vigorous, subject 501 is 41% vigorous, subject 514 is 74% moderate. See `outputs/har70_time_in_zone.png`. This is the strongest, best-supported demonstration of "mobility profiling" in the project, and is what feeds the HAR70+ tab of the app.

*Sit-to-stand transition speed was also attempted (`extract_sit_to_stand_transitions` in `src/har70_features.py`) since it maps directly to the proposal's opening idea, but the dataset only contains 7 real sitting→standing transitions total (one each in 7 of 18 subjects) — a direct consequence of "standing" being 0.22% of all rows. The measurement approach works (scores varied sensibly, 0.09–0.60), but 7 examples isn't enough to support any real conclusion, so this was dropped from the app and is noted here only as attempted-but-data-starved, not as a finding.
...
**Phase 2 status: done** (`training/phase2_sisfall.py`). Stratified 80/20 split, Logistic Regression vs. Decision Tree, both evaluated overall *and* on the SE (elderly) subset alone. Logistic Regression wins overall (93% accuracy, 92% Fall recall) — a realistic, non-suspicious number, unlike cStick. **On the SE-only subset, Fall recall drops to 58%** (vs. 90% for the SA/young-adult-heavy overall set) — the model misses well over a third of real elderly falls it would catch class-wide. Sample size caveat: only 19 elderly fall trials landed in the test split, so the exact number should be read as directional, not precise — but it's a large enough drop to be a real effect, and it's the concrete, measured version of the age-representation bias flagged in our proposal. Confusion matrices in `outputs/sisfall_confusion_matrices.png`. **This is the real accuracy story for the project** — a working model with a known, quantified, honestly-reported weakness, which is far more credible than cStick's inflated 100%.

**Bias mitigation experiment: done** (`training/phase2_sisfall.py`). Your proposal promised to "retrain or adjust the model when significant disparities are found" — so we did: added inverse-frequency sample weighting during training, upweighting the rare (elderly, Fall) combination (only 56 such examples in the training set get a weight of ~16x, vs. ~0.6–1.25x for the common combinations). On the single 80/20 split, this moved SE-only Fall recall from 58% to 63% — real, but based on only 19 test examples, too noisy to trust as the final word.

**Cross-validation robustness check: done** (`training/phase2_sisfall_cv.py`). The single-split estimate above was too noisy to trust on its own (only 19 of the 75 real elderly fall trials were in that one test set) — so we ran 5-fold stratified cross-validation, which evaluates **every one of the 75 real elderly fall trials exactly once** across the 5 folds, instead of relying on one lucky/unlucky split. Result is both bigger and far more stable than the single-split estimate suggested:

- **Baseline (unweighted)**: 54.0% SE-only Fall recall, ±14.9% across folds (highly variable — anywhere from 35% to 71% depending on the split, which is exactly why a single split isn't trustworthy here).
- **Weighted (mitigation)**: 79.0% SE-only Fall recall, ±4.9% across folds (**beats baseline in every single fold**, and consistently, not just by luck).

So the honest, robust finding is: **the reweighting mitigation improves elderly Fall recall from ~54% to ~79%**, a real ~25-point gain, consistently reproduced across 5 different data splits — a much stronger result than the original single-split "58%→63%" estimate implied. It still comes with the same kind of tradeoff: overall Fall precision drops from 91.6% to 83.9% (±~1.7% both ways) — more false alarms generally, in exchange for reliably catching far more real elderly falls. See `outputs/sisfall_cv_se_recall.png` for the per-fold breakdown (every fold shows the same pattern).

The final deployed model (`models/sisfall_model.pkl`) is the weighted Logistic Regression, refit on the **full dataset** (not just one split) now that cross-validation has confirmed the mitigation genuinely generalizes rather than being a single-split fluke. Choosing this over the unweighted baseline is still a values tradeoff (elderly recall vs. general precision) worth your team discussing explicitly, but it's now backed by solid, reproducible evidence rather than a single noisy split.
...
5. ~~Update the proposal's bias/mitigation section with real numbers~~ — done: SisFall SE-subset gap (58% vs. 90% Fall recall) is documented above, along with a real mitigation attempt (sample reweighting, 58%→63%) and its tradeoff (precision cost). The "no dataset here is longitudinal" caveat is documented in the HAR70+ section.
6. ~~Build HAR70+ mobility-profile enhancements~~ — done: time-in-zone profile is solid and feeds the app; sit-to-stand transition speed was attempted and dropped (only 7 real examples, not enough to use).
7. All three algorithms from the proposal are now demonstrated end-to-end, each with honestly-reported strengths and limits. Remaining work: Phase 3 (Streamlit app — 3 tabs: cStick as an interpretability demo, SisFall as the real fall detector with its elderly-recall caveat shown, HAR70+ as the mobility-profile view) and folding these findings back into the written proposal/report.
